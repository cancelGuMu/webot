"""Generic OpenAI-compatible summarization backend.

Uses the ``openai`` Python SDK, so any OpenAI-compatible provider works:
OpenAI, GLM (Zhipu), Moonshot, Qwen, DeepSeek, local vLLM/Ollama, etc.

Base URL and model are fully configurable — set ``OPENAI_BASE_URL`` /
``OPENAI_MODEL`` in .env.  For GLM use:
...
def _parse_summary_from_tool_call(response) -> SummaryResult:
    """Extract SummaryResult from an OpenAI-compatible response.
    ...
    if msg.tool_calls:
        ...
        try:
            ...
            logger.warning(
                "Tool-call arguments malformed — falling through to "
                "fallback strategies. Error: %s", e
            )
    ...
    if content:
        logger.info("Backend returned plain text (no tool call), wrapping as summary")
        return SummaryResult(
...
class OpenAISummarizer(AbstractSummarizer):
    """Summarization via any OpenAI-compatible API.

    Uses tool calling for structured output.  Generic across providers —
    the only provider-specific knob is ``disable_thinking``, which
    subclasses (e.g. DeepSeek) flip on to send ``thinking: disabled``.
    ...
    # Default OpenAI-compatible model ID
    MODEL_DEFAULT = "gpt-4o-mini"

    # Whether to send ``extra_body={"thinking": {"type": "disabled"}}``.
    # DeepSeek requires this; generic OpenAI-compatible providers do not.
    ...
    def _extra_body(self) -> dict | None:
        ...
        Returns ``{"thinking": {"type": "disabled"}}`` when
        ``disable_thinking`` is set, otherwise ``None``.
        """
